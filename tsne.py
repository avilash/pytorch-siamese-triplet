...
def vis_tSNE(embeddings, labels):
	num_sumples = args.tSNE_ns if args.tSNE_ns < embeddings.shape[0] else embeddings.shape[0]
	X_embedded = TSNE(n_components=2).fit_transform(embeddings[0:num_sumples, :])

	fig, ax = plt.subplots()

	x, y = X_embedded[:,0], X_embedded[:,1]
	colors = plt.cm.rainbow(np.linspace(0,1,10))
	sc = ax.scatter(x, y, c=labels[0:num_sumples], cmap=mpl.colors.ListedColormap(colors))
	plt.colorbar(sc)
	plt.savefig(os.path.join('data', args.exp_name, 'tSNE', 'tSNE_' + str(num_sumples) + '.jpg'))
	plt.show()
